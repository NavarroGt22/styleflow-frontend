      ...
      <div className="relative mx-auto flex min-h-screen max-w-7xl flex-col px-5 sm:px-8">
        ...
        <section className="grid flex-1 items-center gap-12 py-14 lg:grid-cols-[1fr_0.9fr] lg:gap-20 lg:py-20">
          <div className="animate-[lp-fade-up_700ms_ease-out_both]">
            <div className="mb-8 flex items-center gap-4">
              ...
              <div>
                ...
                <div className="font-display text-xl tracking-wide text-[#f5f5f4]">{brandName}</div>
              </div>
            </div>
            ...
            <h1 className="font-display max-w-2xl text-5xl leading-[0.98] tracking-[-0.035em] text-balance sm:text-7xl lg:text-[6.8rem]">
              Seu estilo.
              <br />
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center">
              {onBookClick ? (
                <button
                  type="button"
                  onClick={onBookClick}
                  className="group inline-flex cursor-pointer items-center justify-center gap-3 border-none bg-[var(--brand)] px-6 py-4 text-sm font-semibold text-[#111] transition-all hover:-translate-y-1 hover:brightness-110"
                >
          ...
          <div className="relative mx-auto w-full max-w-md animate-[lp-fade_900ms_200ms_ease-out_both] lg:max-w-none">
            <div className="relative aspect-[4/5] overflow-hidden border bg-[var(--lp-card)]" style={{ borderColor: 'var(--lp-border)' }}>
              <div className="absolute inset-0 bg-cover bg-center grayscale" style={{ backgroundImage: heroBg }} />
              <div className="absolute inset-0 bg-[linear-gradient(to_top,rgba(9,11,12,0.95),transparent_58%)]" />
              {sinceLabel && (
                <div className="absolute left-5 top-5 text-[10px] uppercase tracking-[0.32em] text-white/70">{sinceLabel}</div>
              )}
              <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between">
                <div>
                  <p className="mb-2 text-[10px] uppercase tracking-[0.22em] text-[var(--brand)]">A sua cadeira está pronta</p>
                  <p className="font-display text-3xl italic">{salonName}</p>
                </div>
