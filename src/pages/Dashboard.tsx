...
export default function Dashboard() {
  ...
  const [tenantSalons, setTenantSalons] = useState<any[]>(() => user?.salons || []);
  const [showAddUnitModal, setShowAddUnitModal] = useState(false);
  const [newUnit, setNewUnit] = useState({ name: '', slug: '', phone: '' });
  ...
  const fetchProducts = () => {
    if (salon?.id) {
      const token = sessionStorage.getItem('token');
      fetch(apiUrl(`/products/salon/${salon.id}`), {
  ...
  const handlePosSale = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl('/products/sell'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({
          salonId: salon.id,
  ...
  useEffect(() => {
    if (salon?.id) {
      fetch(apiUrl(`/services/${salon.id}`))
        .then(res => res.json())
        .then(data => {
  ...
  const fetchAppointments = () => {
    if (isOwner && salon?.id) {
      const token = sessionStorage.getItem('token');
      fetch(apiUrl(`/appointments/salon/${salon.id}`), {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      .then(res => res.json())
      .then(data => {
  ...
  const fetchAiAdvice = (promptType: string, userMessage?: string) => {
    ...
    const token = sessionStorage.getItem('token');
    fetch(apiUrl(`/financials/salon/${salon.id}/ai-advisor`), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  useEffect(() => {
    ...
    const token = sessionStorage.getItem('token');
    fetch(apiUrl(`/tenants/${user.tenantId}/salons`), {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => (res.ok ? res.json() : []))
      .then((list) => {
        if (!Array.isArray(list) || list.length === 0) return;
        setTenantSalons(list);
        setUser((prev: any) => {
          const updated = { ...prev, salons: list };
          sessionStorage.setItem('user', JSON.stringify(updated));
          return updated;
        });
      })
      .catch((err) => console.error('Erro ao listar unidades:', err));
  }, [isOwner, user?.tenantId]);

  const checkTenantSlug = async (slug: string) => {
    if (slug.length < 3) {
    ...
    try {
      const res = await fetch(apiUrl(`/tenants/check-slug/${slug}`));
      const data = await res.json();
  ...
  const fetchTeamMembers = () => {
    if (isOwner && salon?.id) {
      fetch(apiUrl(`/professionals/${salon.id}`))
      .then(res => res.json())
      .then(data => {
  ...
  const fetchQueueSession = async (professionalId: string) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl(`/queue/session`), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleSkipEntrySubmit = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl(`/queue/entries/${skippingEntryId}/skip`), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleAddWalkInSubmit = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl(`/queue/${queueSession.id}/walkin`), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleReorderEntrySubmit = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl(`/queue/${queueSession.id}/reorder`), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  useEffect(() => {
    if (activeTab === 'queue' && salon?.id) {
      const ws = new WebSocket(wsUrl(`/ws/queue?salonId=${salon.id}`));
  ...
  const fetchTeamTimecards = () => {
    if (isOwner && salon?.id) {
      const token = sessionStorage.getItem('token');
      fetch(apiUrl(`/timecards/salon/${salon.id}?date=${selectedTimecardDate}`), {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      .then(res => res.json())
      .then(data => {
  ...
  const handleClockIn = async () => {
    ...
    try {
      const res = await fetch(apiUrl('/timecards/in'), {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({})
      });
  ...
  const handleClockOut = async () => {
    ...
    setTimecardLoading(true);
    try {
      const res = await fetch(apiUrl('/timecards/out'), {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({})
      });
  ...
  const handleDeleteProfessional = async (id: string) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl(`/professionals/${id}`), {
        method: 'DELETE',
        headers: { 
          'Authorization': `Bearer ${token}` 
        }
  ...
  const handleEditProfessionalSubmit = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl(`/professionals/${editingProfessional.id}`), {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json', 
          'Authorization': `Bearer ${token}` 
  ...
  const exportFinanceCSV = async () => {
    ...
    try {
      await fetch(apiUrl(`/financials/salon/${salon.id}/close`), {
        method: 'POST',
  ...
  const handleCreateSalon = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = sessionStorage.getItem('token');
    const isNewAccount = !user?.tenantId;

    if (isNewAccount && onboardingStep === 1) {
      const slug =
        newAccount.tenantSlug ||
        newAccount.tenantName.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      try {
      ...
      setNewSalon((prev) => ({
        ...prev,
        name: prev.name || newAccount.tenantName,
        slug: prev.slug || slug,
      }));
      setOnboardingStep(2);
      return;
    ...
    const cleanPhone = newSalon.phone.replace(/\D/g, '');

    try {
      const res = await fetch(apiUrl('/establishments'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          name: newSalon.name,
          slug,
          phone: cleanPhone,
          email: user.email || `contato@${slug}.com`,
          tenantName: newAccount.tenantName || newSalon.name,
          tenantSlug: newAccount.tenantSlug || slug,
          primaryColor: newAccount.primaryColor,
          customBrandName: newAccount.customBrandName || undefined,
        }),
      });
  ...
  const handleAddUnit = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('token');
    const slug = newUnit.slug || newUnit.name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const cleanPhone = newUnit.phone.replace(/\D/g, '');

    try {
      const res = await fetch(apiUrl(`/tenants/${user.tenantId}/salons`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ name: newUnit.name, slug, phone: cleanPhone }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      setTenantSalons(nextSalons);
      const updatedUser = { ...user, salons: nextSalons };
      sessionStorage.setItem('user', JSON.stringify(updatedUser));
      setUser(updatedUser);
      setShowAddUnitModal(false);
      setNewUnit({ name: '', slug: '', phone: '' });
      navigate(`/admin/${data.slug}`);
    } catch (err) {
      console.error(err);
      toast.error('Erro na conexão com o servidor');
  ...
  const handleSaveService = async (e: React.FormEvent) => {
    ...
    const url = editingId ? apiUrl(`/services/${editingId}`) : apiUrl('/services');
  ...
  const openScheduleModal = async (service: any) => {
    ...
    try {
      const res = await fetch(apiUrl(`/professionals/${salon.id}`));
      if (res.ok) {
        const data = await res.json();
  ...
  const handleBlockSubmit = async (e: React.FormEvent) => {
    ...
    const token = sessionStorage.getItem('token');
    try {
      const res = await fetch(apiUrl('/appointments/block'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({
          salonId: salon.id,
  ...
  const submitCheckout = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(apiUrl(`/appointments/${checkoutApt.id}/complete`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ 
  ...
  return (
    <div className="min-h-screen p-8 max-w-6xl mx-auto transition-colors duration-300">
        ...
        <div>
          ...
          {isOwner && tenantSalons.length > 0 && (
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {tenantSalons.length > 1 ? (
                <select
                  value={salon?.slug || ''}
                  onChange={(e) => navigate(`/admin/${e.target.value}`)}
                  className="text-sm px-3 py-1.5 rounded-lg border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-800 dark:text-slate-200"
                >
                  {tenantSalons.map((s: any) => (
              ...
              {user?.tenantId && (
                <button
                  ...
                  className="text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/30"
                >
                  + Adicionar unidade
                </button>
              )}
                ...
                {settingsSubTab === 'temas' && (
                  <div className="space-y-6 animate-in fade-in duration-300">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Nome da Conta / Marca</label>
                        <input
                          type="text"
                          value={salonForm.tenantName}
                          onChange={e => setSalonForm({ ...salonForm, tenantName: e.target.value })}
                          className="w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Nome Exibido (opcional)</label>
                        <input
                          type="text"
                          value={salonForm.customBrandName}
                          onChange={e => setSalonForm({ ...salonForm, customBrandName: e.target.value })}
                      ...
                      <div>
                        ...
                        <div className="flex gap-3 items-center">
                          <input
                            ...
                            className="h-11 w-14 rounded-lg border border-gray-300 dark:border-slate-600 cursor-pointer"
                          />
                          <input
                            type="text"
                            value={salonForm.primaryColor}
                            onChange={e => setSalonForm({ ...salonForm, primaryColor: e.target.value })}
                            className="flex-1 px-4 py-2.5 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary outline-none font-mono text-sm"
                          />
                        </div>
                      </div>
                      <div className="sm:col-span-2">
      ...
      {/* MODAL OBRIGATÓRIO: ONBOARDING CONTA + PRIMEIRA UNIDADE */}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-8 text-center animate-in zoom-in-95 duration-300">
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
              {onboardingStep === 1 ? 'Crie sua conta' : 'Primeira unidade'}
            </h2>
            <p className="text-gray-500 dark:text-slate-400 mb-6 text-sm">
              {onboardingStep === 1
                ? 'Passo 1 de 2 — Marca, subdomínio e identidade visual da sua rede.'
                : 'Passo 2 de 2 — Dados operacionais da unidade matriz.'}
            </p>

            <div className="flex gap-2 mb-6">
              <div className={`h-1 flex-1 rounded-full ${onboardingStep >= 1 ? 'bg-primary' : 'bg-gray-200 dark:bg-slate-700'}`} />
              <div className={`h-1 flex-1 rounded-full ${onboardingStep >= 2 ? 'bg-primary' : 'bg-gray-200 dark:bg-slate-700'}`} />
            </div>
            ...
            <form onSubmit={handleCreateSalon} className="space-y-4 text-left">
              {onboardingStep === 1 ? (
                <>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Nome da marca / conta</label>
                    <input
                      required
                      ...
                      onChange={(e) => {
                        ...
                        setNewAccount({ ...newAccount, tenantName: name, tenantSlug: autoSlug });
                        if (autoSlug.length >= 3) checkTenantSlug(autoSlug);
                      }}
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Subdomínio</label>
                    <div className="flex items-center gap-1 text-sm">
                      <input
                        required
                        value={newAccount.tenantSlug}
                        onChange={(e) => {
                          const slug = e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '');
                          setNewAccount({ ...newAccount, tenantSlug: slug });
                          checkTenantSlug(slug);
                        }}
                        type="text"
                        className="flex-1 px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary outline-none"
                        placeholder="barbeariadorafael"
                      />
                      <span className="text-gray-400 whitespace-nowrap">.meusite.com</span>
                    </div>
                    {slugChecking && <p className="text-xs text-gray-400 mt-1">Verificando...</p>}
                    {slugAvailable === true && <p className="text-xs text-emerald-600 mt-1">Subdomínio disponível</p>}
                    {slugAvailable === false && <p className="text-xs text-red-500 mt-1">Subdomínio indisponível</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Cor da marca</label>
                    <input
                      type="color"
                      value={newAccount.primaryColor}
                      onChange={(e) => setNewAccount({ ...newAccount, primaryColor: e.target.value })}
                ...
                <>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Nome da unidade</label>
                    <input required value={newSalon.name} onChange={e => setNewSalon({ ...newSalon, name: e.target.value })} type="text" className="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary outline-none" placeholder="Ex: Unidade Centro" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Link da unidade (opcional)</label>
                    <input value={newSalon.slug} onChange={e => setNewSalon({ ...newSalon, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '') })} type="text" className="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary outline-none" placeholder={newAccount.tenantSlug || 'unidade-centro'} />
                  </div>
                  <div>
              ...
              <div className="flex gap-2 pt-2">
                {onboardingStep === 2 && (
                  <button type="button" onClick={() => setOnboardingStep(1)} className="flex-1 py-3 border border-gray-300 dark:border-slate-600 rounded-xl font-bold text-gray-600 dark:text-slate-300">
                    Voltar
                  </button>
                )}
                <button type="submit" disabled={onboardingStep === 1 && slugAvailable === false} className="flex-1 py-3 bg-primary hover:bg-primary-dark disabled:opacity-50 text-white rounded-xl font-bold transition-all shadow-lg shadow-primary/30">
                  {onboardingStep === 1 ? 'Continuar' : 'Iniciar meu negócio'}
                </button>
              </div>
            </form>
          </div>
      ...
      {/* MODAL: ADICIONAR FRANQUIA / UNIDADE */}
      {showAddUnitModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-8">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Nova unidade</h2>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-6">Adicione uma franquia à mesma conta.</p>
            <form onSubmit={handleAddUnit} className="space-y-4">
              <input required value={newUnit.name} onChange={e => setNewUnit({ ...newUnit, name: e.target.value })} placeholder="Nome da unidade" className="w-full px-4 py-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600" />
              <input value={newUnit.slug} onChange={e => setNewUnit({ ...newUnit, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '') })} placeholder="Slug (ex: unidade-shopping)" className="w-full px-4 py-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600" />
              <input required value={newUnit.phone} onChange={e => setNewUnit({ ...newUnit, phone: e.target.value })} placeholder="Telefone" className="w-full px-4 py-3 border rounded-xl dark:bg-slate-700 dark:border-slate-600" />
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowAddUnitModal(false)} className="flex-1 py-3 border rounded-xl font-bold">Cancelar</button>
                <button type="submit" className="flex-1 py-3 bg-primary text-white rounded-xl font-bold">Criar unidade</button>
              </div>
            </form>
          </div>
        </div>
